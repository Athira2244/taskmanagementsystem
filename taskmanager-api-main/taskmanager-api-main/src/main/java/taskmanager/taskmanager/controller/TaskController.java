...
public class TaskController {
    ...
    public Task createTask(@RequestBody TaskRequest request) {
        ...
        if (request.getChecklistTemplateId() != null) {
            List<taskmanager.taskmanager.model.ChecklistTemplateItem> items = checklistTemplateItemRepo
    ...
    public List<taskmanager.taskmanager.dto.TaskReportDTO> getTaskReport(@PathVariable Integer empId) {
        ...
        for (TaskResponse task : tasks) {
            ...
            List<taskmanager.taskmanager.model.EmpTaskTime> timeLogs = empTaskTimeRepository
                    .findByTaskFkey(task.getTaskId().intValue());
            int totalMinutes = timeLogs.stream().mapToInt(taskmanager.taskmanager.model.EmpTaskTime::getDurationMinutes)
                    .sum();
    ...
    public Map<String, String> updateTask(
        ...
        // Handle Checklist Template Update (Add items if template selected and no items
        // exist)
        // Handle Checklist Template Update (Add items if template selected and no items
        // exist)
        if (request.getChecklistTemplateId() != null) {
            // Simple logic: if a template is selected during update, we add its items.
            // You might want to delete existing items first if you're replacing the
            // checklist.
